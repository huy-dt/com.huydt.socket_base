package com.huydt.socket_client_base;

import com.huydt.socket_client_base.core.ClientConfig;
import com.huydt.socket_client_base.core.SocketBaseClient;
 ...
 *   players                           List players (lobby: APP_SNAPSHOT, room: ROOM_SNAPSHOT)
 *   --- admin commands (require --admin token or admin-auth command) ---
...
public class Main {
    ...
    // Last received APP_SNAPSHOT (lobby state)
    ...
    public static void main(String[] args) throws Exception {
        ...
        client
            .on(ClientEventType.CONNECTED, e ->
                System.out.println("[Event] Connected to " + config.wsUri()))
            ...
            .on(ClientEventType.WELCOME, e -> {
                ...
                if (roomJ != null) System.out.println("[Event] Room     — " + roomJ.optString("name"));
            })
            ...
            .on(ClientEventType.APP_SNAPSHOT, e -> {
                ...
                int players = countArray(e.getPayload(), "players");
                int rooms   = countArray(e.getPayload(), "rooms");
                System.out.printf("[Event] APP_SNAPSHOT   — %d player(s), %d room(s)%n", players, rooms);
            })
            ...
            .on(ClientEventType.ROOM_SNAPSHOT, e -> {
                ...
                if (r != null)
                    System.out.printf("[Event] ROOM_SNAPSHOT  — %s players=%d%n",
                            r.optString("name"), client.getSession().getCurrentRoom() != null
            ...
            .on(ClientEventType.ROOM_LIST, e -> {
                ...
                if (rooms != null) {
                    for (int i = 0; i < rooms.length(); i++) {
                        ...
                        System.out.printf("         • %-20s id=%-12s state=%s players=%d%n",
    ...
    private static void handleCommand(String cmd, String[] parts, SocketBaseClient client) {
        switch (cmd) {
            ...
            // ── Players list — works in both lobby and room ───────────
            case "list-players":
            case "players": {
                com.huydt.socket_client_base.model.RoomInfo room = client.getSession().getCurrentRoom();
                if (room != null) {
                    // Inside a room — show from live local state
                    System.out.printf("  [room: %s] %-24s %-16s %-10s%n",
                            room.name, "ID", "NAME", "STATUS");
                    System.out.println("  " + "-".repeat(56));
                    for (com.huydt.socket_client_base.model.PlayerInfo p : room.getPlayers().values()) {
                        System.out.printf("         %-24s %-16s %-10s%n",
                                p.id, p.name,
                                p.connected ? "online" : "offline");
                    }
                } else if (lastAppSnapshot != null) {
                    // In lobby — show from last APP_SNAPSHOT
            ...
            // ── Admin: rooms ──────────────────────────────────────────
            case "create-room": {
                if (parts.length < 2) { System.out.println("  Usage: create-room <name> [maxPlayers]"); break; }
            ...
            case "broadcast": {
                ...
                client.adminBroadcast(roomId, tag, msgStr != null
                        ? new JSONObject().put("message", msgStr) : null);
                break;
            }
    ...
    private static void printPlayerTable(JSONArray players, String source) {
        ...
        for (int i = 0; i < players.length(); i++) {
            ...
            // Prefer roomName if present, fallback to roomId, then "—"
            ...
            String status = p.optBoolean("connected") ? "online" : "ghost";
    ...
    private static void printConsoleHelp() {
        ...
        System.out.println("  unban      <name>                 Unban by display name");
        System.out.println("  ban-ip     <ip>                   Ban an IP");
        System.out.println("  unban-ip   <ip>                   Unban an IP");
        System.out.println("  ban-list                          Show ban lists");
        System.out.println("  create-room <name> [max]          Create a room");
        System.out.println("  close-room  <roomId>              Close a room");
        System.out.println("  room-state  <roomId> <state>      Set room state");
        System.out.println("              WAITING STARTING PLAYING PAUSED ENDED");
        System.out.println("  broadcast   [roomId|-] <tag> [msg] Broadcast event");
